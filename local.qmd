...
```{r plt-goi-vln-cluster, fig.width = 48, fig.height = 14, fig.fullwidth = TRUE}
...
```{r plt-goi-vln-tissue, fig.width = 32, fig.height = 12, fig.fullwidth = TRUE}
...
```{r subset-srt-goi}
l6.srt <- subset(l6.srt, idents = c("Hypoth", "MBd", "MBv", "Medulla", "Pons", "Thal"))
l6.srt <- FindVariableFeatures(l6.srt, selection.method = "vst", nfeatures = 3500)
l6.srt <- ScaleData(l6.srt)
var.genes <- VariableFeatures(l6.srt)
cell.clusters <- Idents(l6.srt)
...
```{r plt-init-swne, fig.width = 24, fig.height = 14, fig.fullwidth = TRUE}
DimPlot_scCustom(
  l6.srt,
  reduction = "swne",
  pt.size = 0.5,
